...
##### data import function
    def from_csv(self,path):
        ...
        for i in range(len(test.dtypes)):
            column_dtype = test.dtypes.iloc[i]  # Use iloc for positional indexing
            if column_dtype.name[0:3] == 'int' or column_dtype.name[0:5] == 'float':
                if (x[i][0] == "'" or x[i][0] == '"'):
                    x[i] = x[i].replace('\'', '').replace('"', '')
                for j in test[x[i]]:
                    if not (j == 0 or (j > 1000 and j < 2100)):
                        types[i] = column_dtype.name[0:5]
                        break
                    else:
                        types[i] = 'year'
            elif column_dtype.name[0:6] == 'object':
                if (x[i][0] == "'" or x[i][0] == '"'):
                    x[i] = x[i].replace('\'', '').replace('"', '')
                for j in test[x[i]]:
                    if j != 0:
                        # Check various date formats: yyyy/mm/dd, yyyy-mm-dd, dd/mm/yyyy, mm/dd/yyyy
                        # Also handle single digit months and days like 2000/1/1
                        date_patterns = [
                            r'\d{4}[/-]\d{1,2}[/-]\d{1,2}',  # yyyy/mm/dd or yyyy-mm-dd
                            r'\d{1,2}[/-]\d{1,2}[/-]\d{4}',  # dd/mm/yyyy or mm/dd/yyyy
                            r'\d{1,2}[/-]\d{1,2}[/-]\d{2}'   # dd/mm/yy or mm/dd/yy
                        ]
                        is_date = any(re.search(pattern, str(j)) for pattern in date_patterns)
                        if not is_date:
                            types[i] = 'varchar'
                            break
                        else:
...
##### output function : list, print, single_json, multiple_jsons, single_html, multiple_htmls, 6 in total.
    ...
    def output(self, output_method, dict_ = None):
        """
        ...
        if output_method == 'list':
            ...
            if output_method == 'single_html':
                ...
                if dict_ is None:
                    for i in range(instance.view_num):
                        view = instance.tables[instance.views[i].table_pos].views[instance.views[i].view_pos]
                        if 'weekday' in view.output_describe():
                            continue
                        if old_view:
                            order2 = 1
                            order1 += 1
