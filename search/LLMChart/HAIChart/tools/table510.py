...
class Table(object):
    ...
    def getIntervalBins(self, f, num_bins=None): #f: features, num_bins: 可选的分段数量
        """
        ...
        或者根据传入的num_bins参数划分为固定数量的区间

        Args:
            f(Feature): The object of class Feature.
            num_bins(int): 可选参数，如果提供则按照指定的分段数量进行划分
        ...
        if num_bins is not None:
            if type(minTime) != type(datetime.date(1995,10,11)):
                # 处理datetime类型
                time_delta = (maxTime - minTime).total_seconds()
                segment_seconds = time_delta / num_bins
                
                for i in range(num_bins):
                    start_time = minTime + datetime.timedelta(seconds=segment_seconds * i)
                    end_time = minTime + datetime.timedelta(seconds=segment_seconds * (i + 1))
                    if i == num_bins - 1:  # 最后一段确保包含maxTime
                        end_time = maxTime
                    # 只显示年份
                    label = f'{start_time.year}~{end_time.year}'
                    bins.append([label, start_time, end_time, 0])
            else:
                # 处理date类型
                days_delta = (maxTime - minTime).days
                segment_days = max(1, days_delta // num_bins)
                
                for i in range(num_bins):
                    start_date = minTime + datetime.timedelta(days=segment_days * i)
                    end_date = minTime + datetime.timedelta(days=segment_days * (i + 1) - 1)
                    if i == num_bins - 1:  # 最后一段确保包含maxTime
                        end_date = maxTime
                    # 如果 minTime.year 和 maxTime.year 相同，只显示日期，否则显示年份
                    ...
                    bins.append([label, start_date, end_date, 0])
            ...
            for i in range(self.column_num):
                for j in range(self.column_num):
                    ...
                    if fi.type == Type.categorical and fj.type == Type.numerical and fi.ratio == 1.0:
                        charts = []
                        if fj.minmin != '' and fj.minmin > 0 and fi.distinct <= 10 and not (len(fj.name) >= 6 and fj.name[0:4] == 'AVG(' and fj.name[-1] == ')'):
            ...
            for i in range(self.column_num):
                for j in range(self.column_num):
                    ...
                    if fi.type == Type.categorical and fj.type == Type.numerical:# and fj.minmin > 0:
                        charts = [Chart.bar]
                        # if fj.minmin != '' and fj.minmin > 0 and fi.distinct <= 15 and not (len(fj.name) >= 6 and fj.name[0:4] == 'AVG(' and fj.name[-1] == ')'):
                        #     charts.append(Chart.pie)
...
# The following functions deal with different types of data and return new_table
                ...
                for j in range(self.column_num):
                    if self.types[j] == Type.numerical:
                        if isinstance(self.D[i][j], int) or isinstance(self.D[i][j], float) or (isinstance(self.D[i][j], str) and self.D[i][j].isdigit()):#self.D[i][j] != '' and :
                            d[self.D[i][column_id]][sum_column] += float(self.D[i][j])
                        sum_column += 2
            for k in d:
    ...
    def dealWithIntervalBin(self, column_id, begin, end, get_head, get_data, num_bins=None):
        """
        ...
        """
        # 如果有指定分段数，重新生成interval_bins
        if num_bins is not None:
            bins = self.getIntervalBins(self.features[column_id], num_bins)
        else:
            bins = self.features[column_id].interval_bins
            
        for bin in bins:
            bin[3] = 0  # initialize the count of each bin to 0
        bin_num = len(bins)
        interval = self.features[column_id].interval
        
        bin_desc = f"TIME" if num_bins is not None else interval
        new_table = Table(self.instance, True, f'{num_bins} BIN {self.names[column_id]} BY {bin_desc}', '')
                        ...
                        for k in range(self.column_num):
                            if self.types[k] == Type.numerical:
                                if isinstance(self.D[i][k], int) or isinstance(self.D[i][k], float) or (isinstance(self.D[i][k], str) and self.D[i][k].isdigit()):
                                    new_table.D[j][sum_column] += float(self.D[i][k])
                                sum_column += 2
                        break
    ...
    def getClassifyTable(self, classify_id, x_id, f, agg): # f: new_table(function), agg: aggregate
        ...
        begin_id = 0
        for k in range(new_table.classify_num):
    ...
    def dealWithTable(self):
        ...
        for i in range(self.column_num):
            ...
            if self.types[i] == Type.temporal:
                # 生成5段的时间间隔表
                new_tables.append(self.dealWithIntervalBin(i, 0, self.tuple_num, True, True, 5))
                # 生成10段的时间间隔表
                new_tables.append(self.dealWithIntervalBin(i, 0, self.tuple_num, True, True, 10))
        ...
        for i in range(self.column_num):
            ...
            for j in range(self.column_num):
                ...
                if (self.types[j] == Type.categorical and self.features[j].distinct <= 20) or self.types[j] == Type.temporal:
                    ...
                    if len(s) > self.features[j].distinct and ((self.types[j] == Type.categorical and (self.features[i].distinct <= self.features[j].distinct or (self.features[i].distinct <= 6 and self.features[j].distinct <= 6))) or self.types[j]==Type.temporal):
                ...
                if self.types[j] == Type.temporal:
                    # 对于5段的间隔分类
                    new_tables.append(self.getClassifyTable(i, j, lambda *args, **kwargs: self.dealWithIntervalBin(*args, **kwargs, num_bins=5), True))
                    # 对于10段的间隔分类
                    new_tables.append(self.getClassifyTable(i, j, lambda *args, **kwargs: self.dealWithIntervalBin(*args, **kwargs, num_bins=10), True))
